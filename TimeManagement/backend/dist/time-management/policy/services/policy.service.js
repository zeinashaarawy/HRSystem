...
let PolicyService = class PolicyService {
    ...
    async create(policyData) {
        if (policyData.scope === time_policy_schema_1.PolicyScope.DEPARTMENT &&
    ...
    async update(id, updateData) {
        ...
        if (updateData.scope === time_policy_schema_1.PolicyScope.DEPARTMENT &&
            !updateData.departmentId &&
            !policy.departmentId) {
            throw new common_1.BadRequestException('Department ID is required for DEPARTMENT scope');
        }
        if (updateData.scope === time_policy_schema_1.PolicyScope.EMPLOYEE &&
