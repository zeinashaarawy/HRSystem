...
let PolicyEngineService = class PolicyEngineService {
    ...
    constructor(policyModel, attendanceModel, exceptionModel, penaltyModel, overtimeModel, holidayModel, vacationIntegrationService) {
    ...
    async computePolicyResults(attendanceRecord, recordDate, scheduledStartTime, scheduledEndTime, scheduledMinutes) {
        ...
        const isOnLeave = await this.vacationIntegrationService.isEmployeeOnLeave(attendanceRecord.employeeId, recordDate);
        ...
        const punchPolicy = 'FIRST_LAST';
        let workedMinutes = this.calculateWorkedMinutes(attendanceRecord.punches, punchPolicy);
        ...
        const overtimeRecords = await this.computeOvertime(attendanceRecord, policy, overtimeMinutes, workedMinutes, scheduledMinutes || 0, isWeekend || isHoliday, exceptionAdjustments, recordDate);
        return {
    ...
    calculateWorkedMinutes(punches, punchPolicy = 'FIRST_LAST') {
        if (punches.length === 0)
            return 0;
    ...
    calculateLateness(punches, scheduledStart, latenessRule) {
        ...
        const punchTime = firstInPunch.time instanceof Date
            ? firstInPunch.time
            : new Date(firstInPunch.time);
        const scheduledTime = scheduledStart instanceof Date
            ? scheduledStart
    ...
    async computePenalties(attendanceRecord, policy, latenessMinutes, shortTimeMinutes, exceptionAdjustments, recordDate) {
        ...
        if (policy.latenessRule && exceptionAdjustments.latenessMinutes > 0) {
            ...
            if (latenessRule.maxDeductionPerDay &&
    ...
    async computeOvertime(attendanceRecord, policy, overtimeMinutes, workedMinutes, scheduledMinutes, isWeekend, exceptionAdjustments, recordDate) {
        ...
        if (overtimeRule.dailyCapMinutes &&
...
exports.PolicyEngineService = PolicyEngineService = __decorate([
    ...
    __param(5, (0, mongoose_1.InjectModel)(holiday_schema_1.Holiday.name)),
