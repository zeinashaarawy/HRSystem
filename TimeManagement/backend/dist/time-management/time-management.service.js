...
const permission_validation_service_1 = require("./permission/services/permission-validation.service");
const device_sync_service_1 = require("./device/services/device-sync.service");
...
let TimeManagementService = class TimeManagementService {
    ...
    deviceSyncService;
    constructor(attendanceModel, exceptionModel, notificationModel, policyEngineService, scheduleHelperService, repeatedLatenessService, permissionValidationService, deviceSyncService) {
        this.attendanceModel = attendanceModel;
        this.exceptionModel = exceptionModel;
        this.notificationModel = notificationModel;
        this.policyEngineService = policyEngineService;
        this.scheduleHelperService = scheduleHelperService;
        this.repeatedLatenessService = repeatedLatenessService;
        this.permissionValidationService = permissionValidationService;
        this.deviceSyncService = deviceSyncService;
    }
    async recordPunch(dto, isOnline = true) {
        if (!dto.employeeId || !mongoose_2.Types.ObjectId.isValid(dto.employeeId)) {
        ...
        if (!isOnline && this.deviceSyncService && dto.device) {
            await this.deviceSyncService.queueOfflinePunch(dto.employeeId, punchTime, dto.type.toLowerCase(), dto.device || 'unknown', dto.location, dto.rawMetadata);
            return {
        ...
        if (hasInOutPunches) {
            try {
                const scheduled = await this.scheduleHelperService.getScheduledTimes(employeeObjectId, startOfDay);
                if (scheduled.startTime && scheduled.scheduledMinutes) {
                    const result = await this.policyEngineService.computePolicyResults(savedRecord, startOfDay, scheduled.startTime, scheduled.endTime, scheduled.scheduledMinutes);
                    ...
                    if (result.latenessMinutes > 0) {
                        ...
                        if (!existingLateAlert) {
                            await this.sendNotification(dto.employeeId, 'LATE_ARRIVAL', `Late arrival detected: ${result.latenessMinutes} minutes late on ${startOfDay.toDateString()}`);
                            ...
                            try {
                                await this.repeatedLatenessService.trackLatenessIncident(employeeObjectId, lateException._id, result.latenessMinutes, startOfDay);
                                const policy = await this.policyEngineService.getApplicablePolicy(employeeObjectId, startOfDay);
                                if (policy) {
                                    await this.repeatedLatenessService.checkAndEscalateThresholds(employeeObjectId, policy);
                                }
                            }
                            catch (error) {
                                console.error(`Error tracking repeated lateness for employee ${dto.employeeId}:`, error);
                            }
                        }
                    }
                    if (result.shortTimeMinutes > 0) {
                        ...
                        if (!existingEarlyLeaveAlert) {
                            await this.sendNotification(dto.employeeId, 'EARLY_LEAVE', `Early leave detected: ${result.shortTimeMinutes} minutes short on ${startOfDay.toDateString()}`);
    ...
    async getNotifications(employeeId) {
        return this.notificationModel
            .find({ to: new mongoose_2.Types.ObjectId(employeeId) })
            .lean();
    }
    ...
    async createTimeException(employeeId, recordId, reason, assignedToId, type, permissionType, durationMinutes, requestedDate) {
        const exception = new this.exceptionModel({
            ...
            reason,
            type: type || index_1.TimeExceptionType.MISSED_PUNCH,
        ...
        if (permissionType && durationMinutes && requestedDate && this.permissionValidationService) {
            try {
                const validationResult = await this.permissionValidationService.validatePermission(new mongoose_2.Types.ObjectId(employeeId), permissionType, durationMinutes, requestedDate);
                if (!validationResult.valid) {
                    throw new common_1.BadRequestException(`Permission validation failed: ${validationResult.errors.join(', ')}`);
                }
                if (validationResult.valid) {
    ...
    async approveException(exceptionId, approvedBy, notes) {
        ...
        if (exception.permissionType) {
            if (!exception.contractStartDateValidated ||
                !exception.financialCalendarValidated ||
                !exception.probationDateValidated) {
                console.warn(`Permission exception ${exceptionId} approved without full validation. Consider validating before approval.`);
            }
...
exports.TimeManagementService = TimeManagementService = __decorate([
    ...
    __param(5, (0, common_2.Inject)((0, common_2.forwardRef)(() => repeated_lateness_service_1.RepeatedLatenessService))),
    __param(6, (0, common_2.Inject)((0, common_2.forwardRef)(() => permission_validation_service_1.PermissionValidationService))),
    __param(6, (0, common_2.Optional)()),
    __param(7, (0, common_2.Inject)((0, common_2.forwardRef)(() => device_sync_service_1.DeviceSyncService))),
    ...
    __metadata("design:paramtypes", [mongoose_2.Model,
        ...
        permission_validation_service_1.PermissionValidationService,
