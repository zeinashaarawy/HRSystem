...
const renew_assignment_dto_1 = require("../dto/renew-assignment.dto");
...
let ScheduleAssignmentController = class ScheduleAssignmentController {
    ...
    async assign(createDto, req) {
        try {
            if (!createDto.assignedBy) {
                const userId = req.user?.id || req.headers['x-user-id'];
                if (!userId) {
                    throw new common_1.BadRequestException('Unable to determine current user. Please ensure you are logged in.');
                }
                if (!mongoose_1.Types.ObjectId.isValid(userId)) {
                    throw new common_1.BadRequestException(`Invalid user ID format: "${userId}". User ID must be a valid MongoDB ObjectId (24 hexadecimal characters).`);
                }
    ...
    async renewAssignment(id, renewDto) {
        return await this.scheduleAssignmentService.renewAssignment(id, new Date(renewDto.effectiveTo), renewDto.reason);
    }
};
...
__decorate([
    ...
    __metadata("design:paramtypes", [create_schedule_assignment_dto_1.CreateScheduleAssignmentDto, Object]),
...
__decorate([
    (0, common_1.Patch)('scheduling/assignments/:id/renew'),
    (0, roles_decorator_1.Roles)('HR_ADMIN', 'HR Manager', 'SYSTEM_ADMIN'),
    (0, swagger_1.ApiBearerAuth)(),
    (0, swagger_1.ApiOperation)({
        summary: 'Renew/Extend shift assignment',
        description: 'Extend the expiry date (effectiveTo) of a shift assignment. Used when HR wants to renew an expiring assignment.',
    }),
    (0, swagger_1.ApiParam)({ name: 'id', description: 'Assignment ID' }),
    (0, swagger_1.ApiBody)({ type: renew_assignment_dto_1.RenewAssignmentDto }),
    (0, swagger_1.ApiResponse)({
        status: 200,
    ...
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, renew_assignment_dto_1.RenewAssignmentDto]),
    __metadata("design:returntype", Promise)
], ScheduleAssignmentController.prototype, "renewAssignment", null);
