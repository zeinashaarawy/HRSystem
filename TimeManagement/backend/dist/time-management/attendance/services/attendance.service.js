...
let AttendanceService = class AttendanceService {
    ...
    calculateWorkedMinutes(punches, punchPolicy = 'FIRST_LAST') {
        ...
        if (punchPolicy === 'ONLY_FIRST') {
            const firstIn = sortedPunches.find((p) => p.type.toString().toUpperCase() === 'IN' || p.type === index_1.PunchType.IN);
            const firstOut = sortedPunches.find((p) => p.type.toString().toUpperCase() === 'OUT' || p.type === index_1.PunchType.OUT);
        ...
        if (punchPolicy === 'FIRST_LAST') {
            ...
            const lastOut = [...sortedPunches]
                .reverse()
                .find((p) => p.type.toString().toUpperCase() === 'OUT' || p.type === index_1.PunchType.OUT);
        ...
        if (punchPolicy === 'MULTIPLE') {
            ...
            for (const punch of sortedPunches) {
                ...
                else if ((punchType === 'OUT' || punchType === index_1.PunchType.OUT) &&
                    lastInTime) {
