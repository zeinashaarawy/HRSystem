...
export class AuthService {
  ...
  async login(dto: { employeeNumber: string; password: string }) {
    // ✅ Step 1: find employee by employeeNumber and populate role
    ...
    // Extract Role from populated accessProfileId
    let role = '';
    if (employee.accessProfileId && (employee.accessProfileId as any).roles && (employee.accessProfileId as any).roles.length > 0) {
      // Taking the first role for now as the primary role
      role = (employee.accessProfileId as any).roles[0];
    } else {
      // Fallback or default if no role assigned
      role = 'Employee';
    }
