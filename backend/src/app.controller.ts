...
export class AppController {
  ...
  @Put('templates/:id')
  @UsePipes(new ValidationPipe({ transform: true }))
  async updateTemplate(@Param('id') id: string, @Body() dto: UpdateJobTemplateDto) {
  ...
  @Put('jobs/:id')
  @UsePipes(new ValidationPipe({ transform: true }))
  async updateJob(@Param('id') id: string, @Body() dto: UpdateJobRequisitionDto) {
  ...
  @ApiResponse({ status: 400, description: 'Validation failed or consent not given (BR28)' })
  @Post('applications')
  @HttpCode(HttpStatus.CREATED)
  @UsePipes(new ValidationPipe({ transform: true }))
  ...
  @ApiParam({ name: 'id', description: 'Application ID' })
  @ApiResponse({ status: 200, description: 'Application status and history' })
  @ApiResponse({ status: 404, description: 'Application not found' })
  ...
  @ApiResponse({ status: 200, description: 'Rejection notification sent' })
  @ApiResponse({ status: 404, description: 'Application not found' })
  @Post('applications/:id/reject')
  @UsePipes(new ValidationPipe({ transform: true }))
  async notifyCandidate(@Param('id') id: string, @Body() template: RejectionTemplateDto) {
    await this.recruitmentService.sendRejectionNotification(id, template);
    return { message: 'Rejection notification sent successfully', applicationId: id };
  }
  ...
  @ApiResponse({ status: 400, description: 'Interviewer not in panel (BR22)' })
  @ApiResponse({ status: 404, description: 'Interview not found' })
  @Post('interviews/:id/feedback')
  @HttpCode(HttpStatus.CREATED)
  @UsePipes(new ValidationPipe({ transform: true }))
  async submitFeedback(@Param('id') interviewId: string, @Body() dto: SubmitInterviewFeedbackDto) {
  ...
  @ApiResponse({ status: 400, description: 'Application not at offer stage' })
  @Roles('hr_manager')
  @Post('offers')
  @HttpCode(HttpStatus.CREATED)
  @UsePipes(new ValidationPipe({ transform: true }))
  ...
  @ApiResponse({ status: 200, description: 'Offer accepted, onboarding triggered' })
  @ApiResponse({ status: 404, description: 'Offer not found' })
  ...
  @ApiQuery({ name: 'status', required: false, enum: ['submitted', 'in_process', 'offer', 'hired', 'rejected'] })
  ...
  @ApiResponse({ status: 400, description: 'Consent is required (BR28)' })
  @Post('consent')
  @HttpCode(HttpStatus.CREATED)
  @UsePipes(new ValidationPipe({ transform: true }))
  async saveConsent(@Body() dto: { candidateId: string; consentGiven: boolean }) {
    // Consent is handled in createApplication, but this endpoint allows updating consent
    if (!dto.consentGiven) {
      return { message: 'Consent is required for data processing (BR28)' };
    }
