# Payroll Execution Server - Startup Script
# This script ensures you're in the correct directory and starts the server

Write-Host "=== Payroll Execution Server Startup ===" -ForegroundColor Cyan
Write-Host ""
...
# Check if dist folder exists (required for npm start)
if (-not (Test-Path "dist\main.js")) {
    Write-Host "⚠️  WARNING: dist/main.js not found. Building project first..." -ForegroundColor Yellow
    Write-Host ""
    npm run build
    if ($LASTEXITCODE -ne 0) {
        Write-Host "❌ Build failed! Fix errors before starting server." -ForegroundColor Red
        exit 1
    }
    Write-Host ""
}
