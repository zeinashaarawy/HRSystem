...
export default function OnboardingTracker() {
  ...
  const fetchOnboarding = async (employeeId: string) => {
    try {
      setLoading(true);
      // Backend now handles both ObjectId and employee number
      const response = await getOnboardingByEmployee(employeeId);
      setOnboarding(response.data);
      setError(null);
    } catch (err: any) {
      // Don't log or show error if it's a 404 - candidates may not have onboarding yet
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="space-y-4">
          ...
          {onboarding.tasks.map((task, index) => (
            <div
              key={`task-${onboarding._id}-${index}-${task.name || 'task'}-${task.department || ''}`}
